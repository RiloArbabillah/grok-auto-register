...
启用 `sub2api_auto_import` 后，CPA 凭证会先调用 Grok Responses API 做严格预检。只有 HTTP `200` 才会创建或更新 Sub2API 账号；HTTP `402` 会立即拒绝，临时性的 `403`、`408`、`425`、`429`、网络错误和部分 `5xx` 最多重试三次。最终未通过的凭证移动到 `cpa_rejected/`，不会复制到 hotload 目录。
...
管理员密钥只放在被 Git 忽略的 `config.json`。导入成功后的 usage readiness 会报告 `ready`、`payment_required`、`forbidden`、`rate_limited` 或 `pending`，但不会撤销已完成的导入。
...
| `cpa_rejected/xai-*.json` | 未通过 Sub2API Responses API 预检的凭证 |
...
├── sub2api_admin.py            # Sub2API 预检、导入与 readiness 检查
├── cpa_xai/
...
│   ├── probe.py               # Grok Responses API 严格预检
